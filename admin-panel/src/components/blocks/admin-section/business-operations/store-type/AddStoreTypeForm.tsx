// =============================================================
// FILE: AddStoreTypeForm.tsx
// Yeni mağaza türü oluşturma formu
// UpdateStoreTypeForm ile aynı pattern — data prop yok
// =============================================================
...
import { useStoreTypeStoreMutation } from '@/modules/admin-section/business-operations/store-type/store-type.action';
import {
  StoreTypeFormData,
  storeTypeSchema,
...
const AddStoreTypeForm = () => {
  ...
  const {
    ...
    dfSync: {
      ...
      pairs: [
        ...
        { dfField: 'additional_charge_name_df', srcField: (l) => `additional_charge_name_${l}`, validate: false },
      ],
    },
  });
  ...
  const { mutate: StoreTypeStore, isPending } = useStoreTypeStoreMutation();

  const onSubmit = async (values: StoreTypeFormData) => {
    const safeType = normalizeChargeType(values.additional_charge_type);
    ...
    return StoreTypeStore(submissionData as any, {
      onSuccess: () => {
        reset();
        setLastSelectedImages(null);
        setIsFeature(false);
                        ...
                        <Card className="p-2 md:p-6 space-y-2">
                          ...
                          <div>
                            ...
                            <Controller
                              ...
                              render={({ field }) => (
                                <AppSelect
                                  value={normalizeChargeType(field.value) || ''}
                                  onSelect={(value) => {
...
export default AddStoreTypeForm;
