...
class OrderRefundRepository implements OrderRefundInterface
{
    ...
    public function get_order_refund_request(array $filters)
    {
        ...
        return $query->with(['order', 'store.related_translations', 'customer', 'orderRefundReason.related_translations', 'returnShipment'])->latest()->paginate($filters['per_page'] ?? 10);
    }
    ...
    public function get_seller_store_order_refund_request(int $store_id, array $filters)
    {
        ...
        return $query->where('store_id', $store_id)
            ->with(['store', 'customer', 'orderRefundReason', 'returnShipment'])
            ->latest()
