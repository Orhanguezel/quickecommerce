    ...
    public function callback(Request $request)
    {
        ...
        try {
            ...
            if ($result->getStatus() === 'success' && strtoupper((string)$result->getPaymentStatus()) === 'SUCCESS') {
                ...
                // 2026-06-05: iyzico "Onay bazli tahsilat" mode'unda merchant
                // her paymentTransactionId icin Approval gondermek zorunda.
                // Bu icin callback'te paymentItems'i sakla — admin sonradan
                // tek tuk approve eder, tum item'lar gecer.
                $transactionIds = [];
                foreach ((array) $result->getPaymentItems() as $item) {
                    ...
                    if ($tid) {
                        $transactionIds[] = (string) $tid;
                    }
                }
                if (!empty($transactionIds)) {
                    $orderMaster->iyzico_payment_items_json = json_encode($transactionIds);
                }
