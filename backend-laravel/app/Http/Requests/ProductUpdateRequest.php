...
class ProductUpdateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $rules = [
            ...
            "free_shipping" => "nullable|integer|in:0,1",
            ...
            "variants.*.price_input_currency_code" => "nullable|string|size:3",
            "variants.*.price_input_amount" => "nullable|numeric|min:0",
            "variants.*.special_price" => "nullable|numeric|min:0|lte:variants.*.price",
            "variants.*.special_price_input_currency_code" => "nullable|string|size:3",
            "variants.*.special_price_input_amount" => "nullable|numeric|min:0|lte:variants.*.price_input_amount",
    ...
    {
        if (($this->input('status') ?? null) !== 'approved') {
            return false;
        }
        ...
        $product = Product::with('variants')->find($this->input('id'));

        return !$product || !$product->hasSellableVariants();
    }
}
