...
- [X] Ortholand ürün #411 manuel yönetime geçirildi; fiyat/stok sunmayan katalog kaynağının 9 eski mapping’i yedekten sonra kaldırıldı. Ürün ve varyantlar korundu.
- [X] Yonex uyarısı kapatıldı: tam kaynak kataloğunda bulunmayan 94 eski varyant kalıcı `missing_zeroed` durumuna alındı, stokları 0; aktif 110 varyant değişmedi. Son scraper health: 26 kaynak, 0 sorun.
...
- [X] Kullanıcı onayıyla #237 mükerrer 387 TL tahsilat iyzico'dan iptal edildi (`paymentId=4210882766`, sonuç `success`, tutar 387 TRY). Korunacak #238 siparişi `paid` ve onaylı durumda kaldı. Müşteriye iptal tutarı, referansı ve geçerli sipariş bilgisiyle e-posta gönderildi; SMTP gönderimi canlı logda doğrulandı. İptal denetim kaydı: `storage/app/backups/iyzico-cancel-duplicate-237-20260921.json`, e-posta denetim kaydı: `storage/app/backups/customer-email-duplicate-cancel-238-20260921.json`. PR: https://github.com/Orhanguezel/quickecommerce/pull/23 (taslak; #22 üzerine).
...
- [X] Önceki manuel başarılı tarama kayıtlarında `json_size_bytes` eksikti; saatlik freshness kontrolü HeynuT, Yonex ve Ceysport stoklarını karantinaya aldı. Ürün fiyatları kaybolmamıştı.
