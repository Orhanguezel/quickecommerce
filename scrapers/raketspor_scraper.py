#!/usr/bin/env python3
"""
Raketspor Urun Scraper (Ticimax — raketspor.com.tr)
-------------------------------------------------
raketspor.com.tr yalnizca bir TANITIM sitesidir (Next.js, fiyat/stok YOK;
her urun "Satin Al" ile raketspor.com.tr'a yonlendirir). Gercek magaza ve
fiyat/stok verisi **raketspor.com.tr** uzerindedir — Ticimax platformu.
...
def main():
    parser = argparse.ArgumentParser(description="raketspor.com.tr (Ticimax) scraper")
    ...
    output_file = resolve_output("raketspor_products.json")
    print(f"raketspor scraper (Ticimax / raketspor.com.tr) -> {output_file}")
    print("=" * 50)

    if not SCRAPER_URL or not SCRAPER_API_KEY:
        print("HATA: SCRAPER_URL ve SCRAPER_API_KEY env zorunlu "
              "(raketspor.com.tr Cloudflare arkasinda). repo .env kontrol et.")
        sys.exit(2)
    ...
    if args.images and products:
        image_dir = resolve_image_dir("raketspor_images")
    ...
    if not products:
        if blocked:
            print("UYARI: tum sayfalar Cloudflare tarafindan engellendi (error 1010). "
                  "raketspor.com.tr scraper-service IP'sini whitelist'lemeli ya da "
